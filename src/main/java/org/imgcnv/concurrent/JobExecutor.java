...
public class JobExecutor {
    ...
    private JobExecutor() {
        ...
        // es = Executors.newFixedThreadPool(Consts.THREADS);
    ...
    public boolean isReadyJob(Long id) {
        ...
        if (tasks != null) {
            for (FutureObject item : tasks) {
                if (item.getFuture().isDone() == false) {
                    isFinished = false;
        ...
        return (isFinished==true)&&(hasEmptyList==false);
    }
